...
/*   Updated: 2024/06/06 12:48:41 by daduarte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		gradient(int startcolor, int endcolor, int len, int pos)
{
	...
	factor[1] = (float)(((endcolor >> 8) & 0xFF) - ((startcolor >> 8) & 0xFF)) / (float)len;
	factor[2] = (float)(((endcolor & 0xFF)) - ((startcolor) & 0xFF)) / (float)len;
